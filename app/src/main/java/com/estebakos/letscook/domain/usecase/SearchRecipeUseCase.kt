...
class SearchRecipeUseCase @Inject constructor(private val recipesRepository: RecipesRepository) {

    suspend fun execute(query: String, page: Int = 1): Output<List<RecipeUI>> {
        var recipeListOutput: Output<List<RecipeUI>>

        recipesRepository.getLocalRecipesByQuery("%$query%").let { output ->
